...
def test_configure_safe_logging_redacts_exception_text(capsys) -> None:
    ...
    logger = logging.getLogger("myjam.audit.redaction.test")
...
def test_secret_redaction_preserves_uvicorn_access_args_shape() -> None:
    ...
    logger = logging.getLogger("myjam.audit.uvicorn.access.test")
    logger.handlers[:] = [handler]
...
def test_redact_known_myjam_env_secret_values(monkeypatch) -> None:
    monkeypatch.setenv("MYJAM_LASTFM_API_KEY", "myjam_lastfm_secret")
    monkeypatch.setenv("MYJAM_SPOTIFY_CLIENT_SECRET", "myjam_spotify_secret")
    text = redact_secrets("myjam_lastfm_secret myjam_spotify_secret")
    assert "myjam_lastfm_secret" not in text
    assert "myjam_spotify_secret" not in text
    assert text.count("[REDACTED_SECRET]") == 2
