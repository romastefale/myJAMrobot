...
# MYJAM_* is the preferred prefix for new deployments. TR3_*/TR4_* names
# remain supported for Railway compatibility with the previous service.


def _myjam_alias(name: str) -> str | None:
    if name.startswith("TR3_") or name.startswith("TR4_"):
        return "MYJAM_" + name[4:]
    return None


def _candidate_env_names(name: str, legacy: Iterable[str] = ()) -> tuple[str, ...]:
    names: list[str] = []
    for candidate in (name, *legacy):
        alias = _myjam_alias(candidate)
...
def _env(name: str, default: str = "", *, legacy: Iterable[str] = ()) -> str:
    for candidate in _candidate_env_names(name, legacy):
...
def _resolve_data_dir() -> Path:
    ...
    candidates = [Path(raw), Path("/app/data"), Path("/tmp/myjamrobot-data")]
    ...
    fallback = Path("/tmp/myjamrobot-data")
...
def _database_url() -> str:
    for name in _candidate_env_names("TR3_DATABASE_URL", ("DATABASE_URL",)):
        raw = os.getenv(name)
        if raw is not None and raw.strip():
            sqlite_url = _sqlite_or_empty(raw.strip(), source=name)
            if sqlite_url:
                return sqlite_url
...
REQUIRED_ENV_VARS = (
    ("MYJAM_TELEGRAM_BOT_TOKEN/TR3_TELEGRAM_BOT_TOKEN/TELEGRAM_BOT_TOKEN", TELEGRAM_BOT_TOKEN),
    ("MYJAM_SPOTIFY_CLIENT_ID/TR3_SPOTIFY_CLIENT_ID/SPOTIFY_CLIENT_ID", SPOTIFY_CLIENT_ID),
    ("MYJAM_SPOTIFY_CLIENT_SECRET/TR3_SPOTIFY_CLIENT_SECRET/SPOTIFY_CLIENT_SECRET", SPOTIFY_CLIENT_SECRET),
    ("MYJAM_LASTFM_API_KEY/TR3_LASTFM_API_KEY/LASTFM_API_KEY", LASTFM_API_KEY),
    (
        "MYJAM_BASE_URL/TR3_BASE_URL/BASE_URL",
...
_WEBHOOK_SECRET_PURPOSE = b"myjamrobot-webhook-v1"
