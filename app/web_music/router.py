...
_CLIENT_ERROR_BUCKETS: dict[str, deque[float]] = defaultdict(deque)
_CLIENT_ERROR_WINDOW_SECONDS = 60.0
_CLIENT_ERROR_MAX_PER_WINDOW = 60
_CLIENT_ERROR_MAX_KEYS = 1000
...
def _safe_log_field(value: object, limit: int) -> str:
    return redact_secrets(value).replace("\n", " ").replace("\r", " ")[:limit]


def _client_error_allowed(key: str) -> bool:
    now = time.monotonic()
    q = _CLIENT_ERROR_BUCKETS[key]
    cutoff = now - _CLIENT_ERROR_WINDOW_SECONDS
    while q and q[0] < cutoff:
        q.popleft()
    if len(q) >= _CLIENT_ERROR_MAX_PER_WINDOW:
        return False
    q.append(now)
    if len(_CLIENT_ERROR_BUCKETS) > _CLIENT_ERROR_MAX_KEYS:
        _CLIENT_ERROR_BUCKETS.clear()
    return True


@router.post("/api/client-error")
async def client_error(request: Request, payload: ClientErrorPayload) -> dict[str, bool]:
    # Log mínimo e sanitizado. Nunca grava Authorization/initData.
    client_host = (request.client.host if request.client else "unknown")[:80]
    if not _client_error_allowed(client_host):
        return {"ok": True}
    logger.info(
        "WEB_MUSIC_CLIENT_EVENT kind=%s source=%s message=%s extra=%s",
        _safe_log_field(payload.kind or "", 60),
        _safe_log_field(payload.source or "", 40),
        _safe_log_field(payload.message or "", 160),
        _safe_log_field(payload.extra or "", 160),
    )
    return {"ok": True}
