...
def _auth_check_text(result) -> str:
    ...
    if result.ok:
        ...
        if result.error_code == "13":
            lines.append("Fato: erro 13 = assinatura inválida. Verifique se MYJAM_LASTFM_API_KEY e MYJAM_LASTFM_API_SECRET são do mesmo API account e se o secret não foi colado com aspas/espaços.")
        elif result.error_code == "9":
            lines.append("Fato: erro 9 = session key inválida. Gere novamente a MYJAM_LASTFM_SESSION_KEY usando a mesma API key/secret configurada no Railway.")
    return "\n".join(lines)
