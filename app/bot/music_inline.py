...
def _build_inline_icon_revisions() -> dict[str, str]:
    ...
    for kind, filename in _INLINE_ICON_FILES.items():
        ...
        try:
            revisions[kind] = hashlib.sha1(path.read_bytes(), usedforsecurity=False).hexdigest()[:12]
        except Exception:
            revisions[kind] = "0"
    return revisions
...
def _format_inline_music_header(
    ...
    if isinstance(total_plays, int) and total_plays > 0:
        play_prefix = f"♫ {total_plays} · "
    elif isinstance(total_plays, int) and total_plays == 0:
        play_prefix = "♫ · "
    else:
        play_prefix = "♫ "
...
async def _render_tly(item: _PendingInline) -> _InlineRender:
    ...
    artist_raw = str(track.get("artist") or "").strip()
    track_name_raw = str(track.get("track_name") or "").strip()
    lyric_snippet = None  # noqa: F841 - marcador estático do fallback sem letra inline.
