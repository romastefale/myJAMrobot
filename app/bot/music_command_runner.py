...
async def execute_universal_songcharts(
    ...
    *,
    ...
    """Accept a code-owner-only universal chart request.

    The chart source is every valid Last.fm profile in lastfm_profiles. The
    executor rechecks ownership so future routes cannot bypass the Web App/DM
    filters by calling this function directly.
    """
    if not is_code_owner(requester_id):
        logger.info("WEB_UNIVERSAL_SONGCHARTS_REJECTED_NON_OWNER | user_id=%s", requester_id)
        raise MusicCommandError(
            "code_owner_required",
            "Songcharts universal é exclusivo do dono do código.",
            status_code=403,
        )
    period_kind = "month" if str(period or "").lower().startswith("m") else "week"
    _spawn_web_task(
