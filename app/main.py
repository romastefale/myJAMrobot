...
from app.bot.owner_lastfm_scrobble_import import router as owner_lastfm_scrobble_import_router
...
async def _configure_telegram_bot_background() -> None:
    ...
    try:
        ...
        if not _telegram_dispatcher_configured:
            ...
            dispatcher.include_router(owner_lastfm_scrobble_import_router)
    ...
    return JSONResponse(
        {
            ...
            "checks": {
                "database": {
                    "ok": db_ok,
                    "error": "database_check_failed" if db_error else None,
                    "startup_status": _db_startup_status,
                    "startup_error": "database_startup_failed" if _db_startup_error else None,
                },
                "bot_token_configured": bool(TELEGRAM_BOT_TOKEN),
                "dispatcher_configured": _telegram_dispatcher_configured,
                "telegram_startup": {
                    "status": _telegram_startup_status,
                    "error": "telegram_startup_failed" if _telegram_startup_error else None,
                },
            },
        },
